...
// remove the hardcoded image sizes (for fixing dimensions on mobile devices)
add_filter( 'post_thumbnail_html', 'remove_img_dimensions', 10, 3 );
add_filter( 'image_send_to_editor', 'remove_img_dimensions', 10 );

function remove_img_dimensions( $html, $post_id, $post_image_id ) {
...
// PV>
function home_print_thumb(){
	echo '<div class="home-blueprint col-md-7">';
...
function home_print_title(){
	echo '<div class="home-title col-md-5">';

	if ( !is_single() && !in_array('aesop-story-front', get_body_class()) ) :
		the_title( '<h1 class="entry-title title-blueprint"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h1>' );
		echo '<a href="' . esc_url( get_permalink() ) . '" rel="bookmark"><div class="entry-question">';
			the_excerpt( '<h2 class="entry-question">', '</h2>' );
		echo '</div></a>';
	endif;
	echo '</div>';
